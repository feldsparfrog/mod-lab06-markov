...
TEST(Tests, TestPrefixSize) 
{
	...
	vector<string> testsentence = { "Съешь", "же", "этих", "французских", "булок", "да", "выпей", "чаю" };
...
{
	setlocale(LC_ALL, "Russian");
	vector<string> testsentence = { "Ты", "мне", "ворон,", "ты", "мне", "воздух,", "величиною", "с", "кулак", "смотрят", "звёзды" };
...
TEST(Tests, TestPrefixGenerate)
{
	...
	EXPECT_TRUE(GeneratePrefix(testtab).back() == "булок" || GeneratePrefix(testtab).back() == "да" || 
		GeneratePrefix(testtab).back() == "выпей" || GeneratePrefix(testtab).back() == "чаю");
}
