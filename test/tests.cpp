...
#include <vector>
#include "textgen.h"
static TEST(Tests, TestPrefixSize) {
...
static TEST(Tests, TestSuffixSize) {
    setlocale(LC_ALL, "Russian");
    std::vector<std::string> testsentence =
...
static TEST(Tests, TestRandomNumber) {
    int testgen = RandomNumber(20);
    EXPECT_TRUE(testgen >= 0 && testgen <= 20);
}
static TEST(Tests, TestPrefixGenerate) {
    setlocale(LC_ALL, "Russian");
    std::vector<std::string> testsentence =
    { "Съешь", "же", "этих", "французских", "булок", "да", "выпей", "чаю" };
    std::map<prefix, std::vector<std::string>> testtab =
...
static TEST(Tests, TestSuffixChoose1) {
    setlocale(LC_ALL, "Russian");
    std::vector<std::string> testsentence =
    { "ты", "мне", "ворон,", "ты", "мне", "воздух,", "величиною", "с", "кулак",
        "смотрят", "звёзды" };
...
static TEST(Tests, TestSuffixChoose2) {
    setlocale(LC_ALL, "Russian");
    std::vector<std::string> testsentence =
    { "ты", "мне", "ворон,", "ты", "мне", "воздух,", "величиною", "с", "кулак",
        "смотрят", "звёзды" };
...
static TEST(Tests, TestGenerate) {
    setlocale(LC_ALL, "Russian");
    std::vector<std::string> testsentence =
    { "ты", "мне", "ворон,", "ты", "мне", "воздух,", "величиною", "с", "кулак",
        "смотрят", "звёзды" };
